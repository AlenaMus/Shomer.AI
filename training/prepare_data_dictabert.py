"""D9/D10/D12 data-preparation pipeline for DictaBERT 5-class classifier.
...
hate F1 -0.08 for only +0.02 on poor_spelling. D10 is the final model baseline.
The add_noise_to_pool_heavy function remains in augment_noise.py but is NOT called here.

Decision D12: targeted kid-register noise augmentation (augment_kids_noise.py).
  - Adds 1,205 phonetically-corrupted synth_kids rows (synth_kids_noised_d12.jsonl).
  - Noise applied ONLY to synth_kids pool (NOT to sinalab/jigsaw hate/violence rows).
  - This is the critical D11 difference: D11 noised the full pool (confused hate boundary);
    D12 noises only the kid synthetic pool (hate/violence clean signal is preserved).
  - Two tiers: Tier-1 moderate (children_mistakes profile) + Tier-2 heavy (poor_spelling profile).
  - Label-balanced: proportional to original kids pool distribution, NOT offensive-biased.
...
# D12 new source: targeted kid-register noise augmentation (see augment_kids_noise.py)
# 1,205 rows generated by applying phonetic corruption ONLY to synth_kids rows —
# does NOT touch sinalab/jigsaw hate rows, so hate F1 is protected (D11 lesson).
SYNTH_KIDS_NOISED_D12_JSONL = REPO_ROOT / "training/data/interim/synth_kids_noised_d12.jsonl"
...
def main() -> None:  # noqa: C901
    ...
    print("\n  [D12 new source] Loading kid-register noise augmentation...")
    synth_kids_noised_d12_rows = load_jsonl_rows(SYNTH_KIDS_NOISED_D12_JSONL, verbose=True)
    if not synth_kids_noised_d12_rows:
        print("  [WARN] synth_kids_noised_d12.jsonl not found — run training/augment_kids_noise.py first.")
        print("  [WARN] D12 noise injection will be skipped. Slices may remain at D10 levels.")
    ...
    # D12: targeted kid-register noise — ONLY from kids pool (protects hate/violence clean signal)
    if synth_kids_noised_d12_rows:
        n_d12_before = len(all_rows)
        all_rows.extend(normalize_and_filter(synth_kids_noised_d12_rows))
        print(f"  D12 kid-noise injected: {len(all_rows) - n_d12_before} rows")
